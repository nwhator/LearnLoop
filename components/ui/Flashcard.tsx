    ...
    <div className="relative w-full h-80 preserve-3d cursor-pointer perspective-[1000px] mb-8" onClick={handleFlip}>
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden bg-surface-bright rounded-3xl shadow-premium border border-surface-container/50 flex flex-col items-center justify-center p-8 text-center">
          <span className="absolute top-6 left-6 text-sm font-bold text-surface-variant uppercase tracking-wider">Question</span>
          <p className="text-2xl font-headline font-semibold text-surface-on">{card.front}</p>
          <span className="absolute bottom-6 flex items-center gap-2 text-surface-variant text-sm font-medium">
        ...
        >
           <span className="absolute top-6 left-6 text-sm font-bold text-primary-on uppercase tracking-wider opacity-80">Answer</span>
           <div className="flex-1 flex flex-col items-center justify-center">
              <p className="text-xl font-body font-medium text-primary-on text-center mb-8">{card.back}</p>
           </div>
           ...
           <div className="flex gap-4 justify-center" onClick={(e) => e.stopPropagation()}>
              <button 
                ...
                className="bg-primary text-primary-on px-6 py-2 rounded-full font-bold shadow-lg flex items-center gap-2 hover:scale-105 active:scale-95 transition-all"
              >
                 <span className="material-symbols-outlined">check_circle</span> Mastered
              </button>
              <button 
                onClick={(e) => { e.stopPropagation(); handleFlip(); }}
                className="bg-white/20 text-primary-on backdrop-blur-md px-6 py-2 rounded-full font-bold shadow-glass flex items-center gap-2 hover:bg-white/30 transition-all"
              >
