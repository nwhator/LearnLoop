...
export default function DashboardSidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col h-full w-72 fixed left-0 top-0 pt-20 bg-surface-bright border-r border-surface-container z-40 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        ...
        {NAVIGATION_ITEMS.map((item) => {
          ...
          return (
            <Link 
              key={item.href}
              href={item.href}
              className={`flex items-center gap-4 px-6 py-4 rounded-2xl font-bold text-sm tracking-wide transition-all group ${isActive ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-surface-variant hover:bg-surface-container hover:text-surface-on'}`}
            >
      ...
      <div className="p-4 mt-auto border-t border-surface-container bg-white">
        <Link 
          href="/settings"
          className="flex items-center gap-4 px-6 py-4 rounded-2xl font-bold text-xs tracking-widest uppercase text-surface-variant hover:bg-surface-container hover:text-surface-on transition-all outline-none"
        >
