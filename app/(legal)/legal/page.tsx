    ...
    <div className="bg-surface text-on-surface min-h-screen antialiased flex flex-col">
      ...
      <header className="pt-32 pb-16 px-6 md:px-12 bg-white border-b border-surface-container">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="text-5xl md:text-7xl font-black font-headline text-on-surface tracking-tighter leading-tight max-w-4xl mb-8">
             Legal & <span className="text-primary italic font-serif">Trust</span> Center
          </h1>
          <p className="text-xl text-on-surface-variant max-w-2xl leading-relaxed font-bold">
        ...
        <div className="flex flex-col lg:flex-row gap-20">
          ...
          <aside className="lg:w-1/4 hidden lg:block sticky top-28 self-start">
            <div className="bg-white rounded-[2.5rem] p-10 shadow-premium border border-surface-container">
              <h3 className="font-headline font-black text-xl mb-10 flex items-center gap-3 text-on-surface">
          ...
          <div className="lg:w-3/4 space-y-32">
            ...
            <section id="introduction" className="scroll-mt-32">
              <h2 className="text-4xl font-black mb-10 font-headline text-on-surface tracking-tighter">1. Introduction</h2>
              <div className="prose prose-lg max-w-none text-on-surface-variant leading-relaxed space-y-8 font-medium italic border-l-4 border-primary/20 pl-8">
            ...
            <section id="terms" className="scroll-mt-32">
              <h2 className="text-4xl font-black mb-12 font-headline text-on-surface tracking-tighter text-primary">2. Terms of Service</h2>
            ...
            <section id="privacy" className="scroll-mt-32">
              <h2 className="text-4xl font-black mb-12 font-headline text-on-surface tracking-tighter">3. Privacy Policy</h2>
              <div className="bg-white border border-surface-container rounded-[3rem] p-12 space-y-12 shadow-premium">
                <div>
                  ...
                  <p className="text-on-surface-variant leading-relaxed font-bold mb-8">
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-on-surface font-black text-xs uppercase tracking-widest">
...
function LegalNavLink({ id, active, label, onClick }: { id: string, active: boolean, label: string, onClick: () => void }) {
  return (
    <button 
      onClick={onClick} 
      className={`text-left py-4 px-6 rounded-2xl transition-all border ${active ? 'bg-primary text-white border-primary shadow-lg' : 'border-transparent text-on-surface-variant hover:text-on-surface hover:bg-surface'}`}
    >
      {label}
    </button>
  );
...
function LegalCard({ icon, title, children, color, bg }: { icon: string, title: string, children: React.ReactNode, color: string, bg: string }) {
  return (
    <div className="bg-white p-10 rounded-[2.5rem] border border-surface-container shadow-sm hover:shadow-xl transition-all group">
      ...
      <h4 className="font-headline font-black text-2xl mb-4 text-on-surface tracking-tight">{title}</h4>
      <p className="text-on-surface-variant leading-relaxed font-bold text-sm italic">"{children}"</p>
    </div>
  );
}
