    ...
    return (
        ...
        <section className="pt-24 pb-32 px-6 text-center">
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <h1 className="text-6xl md:text-8xl font-black font-headline tracking-tighter text-surface-on leading-[0.9] mb-10">
                ...
                <p className="text-xl text-surface-variant font-bold leading-relaxed max-w-2xl mx-auto">
                ...
                <div className="flex items-center justify-center pt-8">
                    <div className="bg-white border-2 border-surface-container rounded-full p-1.5 flex items-center shadow-lg">
                        <button 
                           ...
                           className={`px-8 py-3 rounded-full text-xs font-black uppercase tracking-widest transition-all ${billingCycle === 'monthly' ? 'bg-surface text-surface-on shadow-inner' : 'text-surface-variant hover:text-surface-on'}`}
                        >
                            Monthly
                        </button>
                        <button 
                           onClick={() => setBillingCycle("yearly")}
                           className={`px-8 py-3 rounded-full text-xs font-black uppercase tracking-widest transition-all relative ${billingCycle === 'yearly' ? 'bg-tertiary text-white shadow-lg' : 'text-surface-variant hover:text-surface-on'}`}
                        >
                            Yearly
        ...
        <section className="bg-surface-container-low py-32 px-6">
            <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
                <div className="space-y-12">
                   <h2 className="text-4xl md:text-5xl font-black font-headline tracking-tighter leading-tight text-surface-on">
                ...
                <div className="relative">
                   ...
                   <div className="bg-white border-2 border-surface-container rounded-[3rem] p-10 shadow-2xl relative z-10 transform lg:rotate-3">
                      <h4 className="font-headline font-black text-2xl mb-6 text-surface-on">Your Intelligence ROI</h4>
                      <p className="text-surface-variant font-bold mb-10 leading-relaxed italic">"Since switching to Learnloop Pro, my memory retention has increased by 78% based on my own analytics. The ROI on $12/mo is insane."</p>
                      ...
                      <div className="flex items-center gap-5 pt-6 border-t border-surface-container">
                         ...
                         <div>
                            <p className="font-black font-headline text-surface-on">David Jenkins</p>
                            <p className="text-[10px] text-surface-variant font-black uppercase tracking-widest pt-1">Medical Resident • Level 24 Scholar</p>
                         </div>
                      </div>
                   </div>
                </div>
...
function PriceCard({ title, price, period, description, features, cta, href, featured = false }: any) {
  return (
    <div className={`flex flex-col p-10 rounded-[3rem] border transition-all duration-500 overflow-hidden relative shadow-sm group ${featured ? 'bg-surface-on text-surface scale-[1.05] border-transparent shadow-premium' : 'bg-white border-surface-container hover:shadow-xl hover:translate-y-[-8px]'}`}>
        {featured && (
        ...
        <div className="flex flex-col gap-8 mb-12">
            <div>
                <h3 className={`font-black font-headline text-2xl tracking-tight ${featured ? 'text-white' : 'text-surface-on'}`}>{title}</h3>
                <p className={`text-sm mt-3 font-medium ${featured ? 'text-surface/70' : 'text-surface-variant'}`}>{description}</p>
            </div>
            
            <div className="flex items-end gap-1">
                <span className={`text-6xl font-black font-headline tracking-tighter ${featured ? 'text-white' : 'text-surface-on'}`}>{price}</span>
        ...
        <Link 
           href={href} 
           className={`w-full py-5 rounded-full text-center font-black uppercase text-xs tracking-[0.2em] transition-all active:scale-95 shadow-lg ${featured ? 'bg-primary text-white hover:brightness-110' : 'bg-surface text-surface-on border border-surface-container hover:bg-surface-on hover:text-surface shadow-inner'}`}
        >
            {cta}
        </Link>
    </div>
  ...
  return (
    <div className="flex items-start gap-8 group">
       ...
       <div className="space-y-2">
          <h4 className="text-2xl font-black font-headline text-surface-on group-hover:text-primary transition-colors">{title}</h4>
          <p className="text-surface-variant font-medium leading-relaxed italic border-l-2 border-surface-container pl-6 opacity-80">"{description}"</p>
       </div>
    </div>
  );
}
